...
**Realtime monitoring** (library setting): watches the root folder for adds/renames/deletes and queues a path-scoped scan after a short debounce. It needs a filesystem that supports inotify (Linux) or equivalent (local ext4/xfs, typical Docker bind mounts to local disks). NFS, CIFS/SMB, and some FUSE/remote shares often cannot create watches - scans and auto-scan still work. K7 logs a Warning once with the failure reason, then retries quietly. Turn realtime monitoring off on those libraries and rely on **Auto scan interval**, or mount media the same way as a library that already shows `Started realtime monitor ... (N directory watches)`. Large local trees may also need higher host `fs.inotify.max_user_watches` / `max_user_instances`.
