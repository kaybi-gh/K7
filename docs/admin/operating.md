...
Users can enable extra shortcuts under Settings -> Navigation (server default: Admin -> Experience -> Navigation). Off by default. Administrators can pin admin sidebar pages as a separate shortcut type (Admin page). Those shortcuts are dropped for accounts without CanAccessAdmin. The main Home / Explore / My Space bar does not change. On desktop the shortcuts are either a Home carousel of small category cards or a thin text bar under the top menu. Shortcuts that do not fit the bar width stay in a More menu. Bar pages are Home, Explore, My Space, Settings (including admin), and/or media pages (movies, series, music, catalog, search). Never auth or the player. The Home row never appears on the Explore group grid. Row pages are Home and/or Explore group feeds (`/explore?library-group=`). Each layout chooses desktop, phone, and/or TV (default: desktop and TV). The bar is desktop only. On phones, only the Home row is used (never Explore feeds). On TV a bar placement falls back to the Home row.

Shortcuts are library groups, collections, playlists (including dynamic playlists), or an allowlisted app page (My Space, Search, Settings, ...). The Downloads app page shortcut is offered on native apps only (hidden on the web client). Legacy filtered-catalog shortcuts still open if they were saved earlier. Reorder them by dragging the handle (or with up/down arrows on the handle). The Home row title is optional (empty uses Shortcuts in the interface language). Each shortcut card can match its target (icon, cover, color) or override those. Users override the server default and can reset.
