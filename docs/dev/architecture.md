...
TV Home and Explore keep MediaCards mounted in a shown shelf. TV vertical shelves grow a mounted row range and do not re-render on D-pad until a new shelf must mount. Explore row components stay mounted so genre/home feeds prefetch while their carousel DOM is parked. TV Home updates the hero via `HomeTvHero.ApplyFocusedItem` so D-pad moves do not re-render every carousel. Title and meta follow focus immediately. Backdrop JPEG decode waits until focus has settled (`TvHeroFocusSettle`, about 200 ms) so skipped cards are not fetched. TV Home and Explore wire MediaCard `OnFocused` through `SuppressRenderEventHandler` so a D-pad move does not `StateHasChanged` the feed. Delayed `focusFirst` retries keep the card the user already reached instead of jumping back to the first MediaCard. TV SerieSeason does the same with `SerieSeasonTvHero.ApplyFocusedEpisode` and `MediaPageBackdrop.ApplyFocusedImage` (Medium stills, decode then fade, same settle window). Explore TV hero cancels in-flight `GetMediaAsync` when focus moves on. Hero backdrops decode then fade in at Medium so Movie/Serie can reuse the Home bitmap. Person backdrops rotate on a timer. Filmography focus does not change them. MusicArtist uses a static portrait color wash. TV library browse (`LibraryGroupView`) keeps D-pad focus off the parent render and lets `K7VirtualGrid` own scroll so Virtualize does not jump to the top. On TV the poster grid is capped at eight columns so tiles stay larger. Virtualize overscan is four rows. JPEG warmup is the mounted window plus a short lookahead, not whole cached pages. Grid D-pad walks the logical catalog (row index). Virtualize only mounts a window and fetches that range in the background, same as mouse-wheel on desktop. D-pad also prefetches ahead of focus because it can outrun Virtualize scroll. Unloaded slots are empty tiles until their page arrives. Fast scroll does not cancel in-flight pages (that left empty tiles stuck). Jumping a letter cancels out-of-range fetches and only loads that window. When a page arrives, mounted placeholder slots become MediaCards in place without rebuilding the virtualize window. It only exits to the toolbar / navbar at scroll top. Filter and sort overlays (`K7Menu`, `K7Select`) stay hidden until placed, skip re-anchor while open, and on TV open under the toolbar control.
...
Android music uses two Media3 ExoPlayers in `K7MediaLibraryService` (session player + idle player) so crossfade and gapless can overlap. Gapless prebuffer prepares the next URL while `CurrentTrack` is still the outgoing title, so the idle player stamps MediaItem metadata from that source `IndexedFileId` / `MediaId` rather than the outgoing queue row. Otherwise Android Auto keeps the first cover and title through the second song. The Media3 session binds to the incoming player as soon as that source is prepared, so the notification title is the incoming track. In-app chrome (title, cover, rating) stays on `CurrentDisplayedTrack` (the outgoing track) until the blend ends. `CurrentPlayingTrack` is already the incoming title. Lock-screen Next skips that title. Previous returns to the outgoing (last notification) title instead of seeking the dying player. After the blend, the incoming player is promoted in place (`ForwardingSimpleBasePlayer.setPlayer`). Radio then prepends already-played K7 queue rows and appends upcoming ones onto that single MediaItem so Android Auto previous can leave the current title. The next track is not reloaded onto the freed session player. A user skip during the blend cancels the fade and applies the new source. Phone Bluetooth (AVRCP, not Android Auto) skip uses the single-item intercept path: reuse the prebuffered next source when possible so the idle ExoPlayer is promoted, otherwise hard-cut (`Stop` + new `MediaItem`) so A2DP does not keep the old AudioTrack while MediaSession already shows the next title. In-app Next on a radio uses the same hard-cut. Seeking a native Media3 playlist from `SourceChanged` used to change artwork while audio stayed on the previous URI. Android Auto skip still uses the ExoPlayer playlist as source of truth (same idea as Tempus / Jellyfin). Next/previous seek the Media3 timeline, then `IAudioPlayerService` copies the current index without reloading the stream. Car radios still fast-start on a small first batch so playback is not blocked on a large fetch. Android Auto gets the first playable URI as soon as it exists, plus any extra URIs that resolve within a short budget. Remaining radio rows append after the player is ready. Media3 then `setMediaItems` the returned list, which can wipe extras, so the service re-reads the live player timeline and refills until it matches the K7 queue. The session player must stay IDLE or ENDED while that batch is resolving (empty Media3 timeline). Advertising READY so lock-screen Next stays tappable used to crash `GetState` and Android Auto showed "Unable to load your selection". Refill then appends to the Media3 playlist. In-app radios keep a single MediaItem so phone Next does not go through that playlist. Android Auto can bind this service without `MainActivity`. The service applies the server URL and restores the session before browse/play so the car does not depend on opening the phone app first. Headless restore also enables `AudioPlaybackProgressTracker` (admin active streams, scrobbles, history) and flushes `PendingPlaybackEvents` without waiting for Blazor paint. Android Auto custom actions (shuffle, repeat, 0-5 star cycle mapped to the 0-10 user rating) go through `IAudioPlayerService` and `IRatingService`. Media3 shuffle/repeat commands are forwarded the same way so the car never uses ExoPlayer's own shuffle order. A brief `NetworkAccess.None` while Bluetooth comes up must not wipe a still-valid Bearer. Headless restore retries until an online token is ready, and a 401 from ExoPlayer refreshes then prepares again. Windows music stays on WebView2 / Web Audio. iOS uses dual `AVPlayer`.
...
Offline playback progress and ratings are queued in `PendingPlaybackEvents` with `IdentityUserId`. `PlaybackSyncService` flushes them after first Blazor paint (past select-profile / splash), or immediately after a headless Android Auto session restore, for the currently **online** authenticated user. Rows without a user id are dropped and never sent.
