...
TV Home updates the hero via `HomeTvHero.ApplyFocusedItem` so D-pad moves do not re-render every carousel. TV SerieSeason does the same with `SerieSeasonTvHero.ApplyFocusedEpisode` and `MediaPageBackdrop.ApplyFocusedImage` (Medium stills, decode then fade). Hero backdrops decode then fade in at Medium so Movie/Serie can reuse the Home bitmap. Person backdrops rotate on a timer. Filmography focus does not change them. MusicArtist uses a static portrait color wash.
