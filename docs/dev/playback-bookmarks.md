...
- No item bookmark on the next episode (user has not started it)
...
| `HomeFeedContinueWatchingStrategy` | Queries `PlaybackBookmarks`, backfills missing next episodes, expires stale series bookmarks |
| `LiteMediaProjectionService` | Merges item bookmarks into `UserMediaStateDto` for position/progress overlay |

## DTO mapping

`UserMediaStateMappings.ToUserMediaStateDto` accepts an optional `ItemPlaybackBookmark` to overlay `LastPlaybackPosition` and `ProgressPercentage` on the DTO without storing them on the entity.
...
`PlaybackBookmarkServiceTests` (complete, dismiss, expire, late-season refresh, backfill, shared profile, batch next-up), `SeriesPlaybackBookmarkRefreshEventHandlerTests`, `ContinueWatchingEligibilityTests`, and related handler tests in `tests/Application.UnitTests`.
