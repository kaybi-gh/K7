...
public record BeginTwoFactorSetupCommand : IRequest<TwoFactorSetupDto>;

public class BeginTwoFactorSetupCommandHandler(
    IApplicationDbContext context,
    IUser currentUser,
    IIdentityService identityService,
    IAuthenticationSettings authSettings) : IRequestHandler<BeginTwoFactorSetupCommand, TwoFactorSetupDto>
{
    public async Task<TwoFactorSetupDto> Handle(BeginTwoFactorSetupCommand request, CancellationToken cancellationToken)
    {
        ...
        var setup = await identityService.BeginTwoFactorSetupAsync(identityUserId);
