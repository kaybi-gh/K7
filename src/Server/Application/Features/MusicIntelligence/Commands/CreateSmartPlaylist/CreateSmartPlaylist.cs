...
public class CreateSmartPlaylistCommandHandler(IMusicIntelligenceService musicIntelligenceService)
    : IRequestHandler<CreateSmartPlaylistCommand, List<Guid>>
{
    ...
    {
        return await musicIntelligenceService.CreatePlaylistFromPromptAsync(request.Prompt, request.Count, cancellationToken);
    }
}
