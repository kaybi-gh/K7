...
public class GetSimilarTracksQueryHandler(IAudioMuseAiService audioMuseAiService)
    : IRequestHandler<GetSimilarTracksQuery, List<Guid>>
{
    ...
    {
        return await audioMuseAiService.GetSimilarTracksAsync(request.TrackId, request.Count, cancellationToken);
    }
}
