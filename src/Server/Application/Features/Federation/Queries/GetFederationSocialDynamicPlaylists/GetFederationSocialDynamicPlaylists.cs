...
namespace K7.Server.Application.Features.Federation.Queries.GetFederationSocialDynamicPlaylists;

public record GetFederationSocialDynamicPlaylistsQuery(string? ClientId, string? ViewerAssertion, Guid OriginUserId)
    : IRequest<IReadOnlyList<FederatedDynamicPlaylistDto>>;

public class GetFederationSocialDynamicPlaylistsQueryHandler(
    ...
    : IRequestHandler<GetFederationSocialDynamicPlaylistsQuery, IReadOnlyList<FederatedDynamicPlaylistDto>>
{
    public async Task<IReadOnlyList<FederatedDynamicPlaylistDto>> Handle(
        GetFederationSocialDynamicPlaylistsQuery request,
        CancellationToken cancellationToken)
    {
        ...
        if (!await visibilityEvaluator.IsFederationSocialEnabledAsync(
                FederationContentType.DynamicPlaylists, outbound: true, peer.Id, cancellationToken))
            return [];
        ...
        if (!await visibilityEvaluator.CanViewFederatedAsync(
                ...
                privacy.Share.DynamicPlaylists,
                cancellationToken: cancellationToken))
            return [];
        ...
        var dtos = new List<FederatedDynamicPlaylistDto>();
        foreach (var playlist in playlists)
        {
            ...
            dtos.Add(new FederatedDynamicPlaylistDto
            {
                Id = playlist.Id,
