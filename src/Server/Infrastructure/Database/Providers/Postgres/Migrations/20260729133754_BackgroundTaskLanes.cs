...
namespace K7.Server.Infrastructure.Database.Providers.Postgres.Migrations
{
    ...
    /// </summary>
    /// <remarks>
    /// The scaffolded version renamed Priority to WorkClass, which would have left the old 0..6 values in
    /// a column whose values are scheduling weights (100..400), and defaulted every row to lane 0. The new
    /// columns are therefore backfilled explicitly here. Task name is the backfill signal for WorkClass
    ...
    public partial class BackgroundTaskLanes : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Peer isolation used to be encoded in the group name.
            migrationBuilder.Sql("""
                UPDATE "BackgroundTasks"
                SET "FederationPeerId" = CAST(substring("ConcurrencyGroup" from 12) AS uuid)
                WHERE "ConcurrencyGroup" LIKE 'federation:%'
                  AND substring("ConcurrencyGroup" from 12) ~ '^[0-9a-fA-F-]{36}$';
                """);
